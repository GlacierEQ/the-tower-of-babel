...
def main() -> int:
    ...
    if args.command == "benchmark":
        ...
        _print(report)
        return 1 if any(row.get("status") == "FAILED" for row in report["results"]) else 0
    if args.command == "proof-report":
        build_path = Path(args.build_report)
        build_report = json.loads(build_path.read_text(encoding="utf-8")) if build_path.is_file() else {}
        report = build_proof_report(registry, build_report)
    ...
    if args.command == "receipt":
        ...
        _print(payload)
        return 0 if payload["registry_valid"] and payload["integrity_valid"] else 1
    if args.command == "report":
        maturity_path = Path("generated/maturity.json")
        _print(json.loads(maturity_path.read_text(encoding="utf-8")))
        return 0
    if args.command == "megamind-map":
        path = Path("generated/megamind.technology-map.json")
