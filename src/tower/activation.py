...
def resolve_activation(technology: Mapping[str, Any], requested: ActivationMode | str, *, external_effects: bool = False) -> ActivationDecision:
    ...
    if evidence_state in {"tested", "benchmark", "integrated", "production_reference", "formally_verified"} and proof_class:
        return ActivationDecision(technology_id, mode, True, mode, "promotion-proof-class-present")
    return ActivationDecision(technology_id, mode, False, ActivationMode.EXECUTE if declared_tool and declared_execution else ActivationMode.INSPECT, "promotion-requires-earned-evidence", ("evidence_state", "proof_class"))
def activate_execution(technology: Mapping[str, Any], *, external_effects: bool = False) -> dict[str, Any]:
    ...
    if not decision.allowed:
        return {"technology_id": decision.technology_id, "status": "ACTIVATION_BLOCKED", "decision": decision}
    from .build import build_floor
    result = build_floor(dict(technology))
    return {**result, "activation": {"mode": ActivationMode.EXECUTE.value, "reason": decision.reason, "governance": "shaping-and-audit"}}
def activation_surface(technology: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {"technology_id": _text(technology, "id") or "UNIDENTIFIED_TECHNOLOGY", "available": [d.effective_mode.value for d in decisions if d.allowed], "decisions": [{"requested": d.requested.value, "allowed": d.allowed, "effective_mode": d.effective_mode.value, "reason": d.reason, "required_proof": list(d.required_proof), "blocked_capabilities": list(d.blocked_capabilities)} for d in decisions]}
