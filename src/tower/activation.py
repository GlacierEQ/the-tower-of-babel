...
def resolve_activation(
    ...
    *,
    ...
    try:
        mode = (
            requested
            if isinstance(requested, ActivationMode)
            else ActivationMode(requested)
        )
    ...
    declared_execution = isinstance(execution, dict) and bool(
        _text(execution, "ci_tier")
    )
    if mode is ActivationMode.EXECUTE:
        if declared_tool and declared_execution:
            return ActivationDecision(
                technology_id, mode, True, mode, "declared-execution-boundary-present"
            )
        missing = tuple(
            name
            for name, present in (
    ...
    return ActivationDecision(
        ...
        if declared_tool and declared_execution
        else ActivationMode.INSPECT,
        "promotion-requires-earned-evidence",
        ("evidence_state", "proof_class"),
    )


def activate_execution(
    technology: Mapping[str, Any], *, external_effects: bool = False
) -> dict[str, Any]:
    decision = resolve_activation(
        technology, ActivationMode.EXECUTE, external_effects=external_effects
    )
...
def activation_surface(technology: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "technology_id": _text(technology, "id") or "UNIDENTIFIED_TECHNOLOGY",
        "available": [d.effective_mode.value for d in decisions if d.allowed],
        "decisions": [
            {
                "requested": d.requested.value,
                "allowed": d.allowed,
                "effective_mode": d.effective_mode.value,
                "reason": d.reason,
                "required_proof": list(d.required_proof),
                "blocked_capabilities": list(d.blocked_capabilities),
            }
            for d in decisions
        ],
    }
