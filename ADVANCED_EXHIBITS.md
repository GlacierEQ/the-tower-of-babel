...
| **Lua** | Capability-table sandbox with frozen environment — frozen metatable environment, pcall capability dispatch, FNV-1a receipt digest | `tested` / `behavioral` | [`advanced_sandbox_capability_table.lua`](languages/lua/advanced_sandbox_capability_table.lua) |
...
| **Java** | Bounded concurrent work queue with receipts — ArrayBlockingQueue bounds, ThreadPoolExecutor abort policy, SHA-256 receipt chain | `tested` / `behavioral` | [`advanced_bounded_work_queue.java`](languages/java/advanced_bounded_work_queue.java) |
| **Kotlin** | Bounded coroutine supervisor with isolated failures — supervisorScope isolation, fixed thread fan-out, SHA-256 digest validation | `tested` / `behavioral` | [`advanced_coroutine_supervisor.kt`](languages/kotlin/advanced_coroutine_supervisor.kt) |
...
| **CUDA C++ / PTX (gated kernel runtime)** | Reference attention tile simulation with an explicit CUDA admission gate — Host-side reference math only until a real nvcc-built and GPU-executed kernel receipt exists | `illustrative` / `illustrative` | [`advanced_nvidia_flash_attention_kernel.cu`](languages/cuda/advanced_nvidia_flash_attention_kernel.cu) |
...
| **eBPF** | Kernel syscall sentinel — kernel syscall execve sentinel | `tested` / `behavioral` | [`advanced_syscall_sentinel.bpf.c`](languages/ebpf/advanced_syscall_sentinel.bpf.c) |
