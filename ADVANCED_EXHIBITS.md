...
| **CUDA C++ / PTX (gated kernel runtime)** | Gated GPU-kernel path — host-side reference simulation; real CUDA execution not established | `illustrative` / `illustrative` | [`advanced_nvidia_flash_attention_kernel.cu`](languages/cuda/advanced_nvidia_flash_attention_kernel.cu) |
...
| **JAX + XLA (gated autodiff runtime)** | Gated autodiff/mesh path — pure-Python reference; JAX/XLA execution not established | `illustrative` / `illustrative` | [`advanced_grok_distributed_mesh.py`](languages/jax/advanced_grok_distributed_mesh.py) |
...
| **RHL-Quant reference quantizer (unbenchmarked)** | Reference ternary/residual quantization experiment — accelerator, model-scale, and quality claims not established | `illustrative` / `illustrative` | [`advanced_rhl_quant_engine.py`](languages/rhl_quant/advanced_rhl_quant_engine.py) |
