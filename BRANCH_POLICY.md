...
5. **Retirement requires proof, not an invented approval gate.** Only provider-read-back `UNIQUE_CONTRIBUTION=0` can establish that a derivative donor is fully drained. Do not substitute merged/closed state, age, patch equivalence, current-main containment, CI success, replay (successful re-execution of an implementation or event sequence), synthesis (successful construction of a combined implementation or artifact from multiple inputs), or a generic approval ritual for that proof. At most, some of those results demonstrate execution or overlap; none accounts for every unique source, event, edge, contradiction, provenance, mechanism, unresolved dependency, receipt, or authority-domain fact. Where the Operator has given a controlling instruction for the work, routine forward repair proceeds under that instruction rather than introducing a new per-retirement approval requirement.
