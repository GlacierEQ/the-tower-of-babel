...
| 16 | **Triton** `.py` | kernel_language | Defines high-throughput GPU kernels with Python syntax and compiler-managed block programming. | Fused attention, normalization, quantization, MoE routing, and custom inference operations. | Use after profiling identifies a GPU kernel boundary that framework operators cannot express efficiently. | It provides programmable GPU performance with less boilerplate than handwritten CUDA. | `hardware_gated` / `hardware` | [easy_vector_add.py](languages/triton/easy_vector_add.py) | [advanced_fused_attention.py](languages/triton/advanced_fused_attention.py) |
| 17 | **Mojo** `.mojo` | language | Combines Python-like authoring with MLIR-based systems programming, SIMD, and accelerator-oriented kernels. | AI runtimes, custom kernels, tensor transformations, and performance-sensitive Python ecosystem extensions. | Use when Python ergonomics and native AI-kernel performance must coexist. | It exposes ownership, vectorization, and compiler specialization through an MLIR toolchain. | `toolchain_gated` / `compile` | [easy_simd.mojo](languages/mojo/easy_simd.mojo) | [advanced_tpu_tensor_kernel.mojo](languages/mojo/advanced_tpu_tensor_kernel.mojo) |
| 18 | **ONNX** `.onnx` | graph_format | Represents machine-learning computation graphs independently of the training framework. | Model exchange, inference deployment, graph optimization, validation, and hardware portability. | Use when a trained model must move across frameworks, runtimes, or accelerators. | Its typed graph and operator sets create an inspectable deployment contract. | `tested` / `behavioral` | [easy_linear_model.py](languages/onnx/easy_linear_model.py) | [advanced_moe_router.py](languages/onnx/advanced_moe_router.py) |
